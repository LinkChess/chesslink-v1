...
void setup() {
  ...
  // configure leds 
  FastLED.addLeds<NEOPIXEL, DATA_PIN>(leds, DISPLAY_LEDS);
  ...
  // Create a rainbow effect at startup
  FastLED.setBrightness(30);
  for (int i = 17; i < DISPLAY_LEDS; i++) {
    // Map each LED to a different hue (0-255) across the rainbow
    leds[i] = CHSV(i * 255 / DISPLAY_LEDS, 255, 255);
  }
  FastLED.show();
  delay(1000); // Display the rainbow for 1 second
...
void loop() {
  ...
  for (int i = 16; i < DISPLAY_LEDS; i++) {
    // Map each LED to a different hue (0-255) across the rainbow
    leds[63 - i] = CHSV(i * 255 / DISPLAY_LEDS, 255, 100);
  }
  FastLED.show();
  // FastLED.setBrightness(255);
  ...
  // char boardNum[2] = {region, '\0'}; // Create a proper null-terminated string from the char
  strcat(latestFEN, region);
  strcat(latestFEN, ":");
