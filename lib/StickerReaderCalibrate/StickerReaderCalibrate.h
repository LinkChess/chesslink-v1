...
class StickerReader {
public:
    StickerReader(int controlA0, int controlA1, int controlA2, int controlA3, int sensorNumber, CRGB &led);
    ...
    int checkAmbientChange(int threshold = 6); // New method: returns 1 (placed), -1 (removed), 0 (no change)
...
private:
    ...
    CRGB &_led;  // Changed to a reference
